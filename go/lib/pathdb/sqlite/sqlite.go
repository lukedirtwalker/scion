...
func insertInterfaces(ctx context.Context, tx *sql.Tx,
	...
	for _, as := range ases {
		...
		for idx, hop := range as.HopEntries {
			...
			if err != nil {
				return serrors.WrapStr("Failed to extract hop field", err)
			}
			if hof.ConsIngress != 0 {
				_, err = stmt.ExecContext(ctx, ia.I, ia.A, hof.ConsIngress, segRowID)
				if err != nil {
					return serrors.WrapStr("Failed to insert Ingress into IntfToSeg", err)
				}
			}
			...
			if idx == 0 && hof.ConsEgress != 0 {
				...
				if err != nil {
					return serrors.WrapStr("Failed to insert Egress into IntfToSeg", err)
				}
			}
		}
	}
