...
type FetchingProvider struct {
	...
	Requests func(reqType, trigger, result string) metrics.Counter
}
...
func (p FetchingProvider) GetChains(ctx context.Context, query ChainQuery,
	...
	if err != nil {
		logger.Info("Failed to get chain from database",
			"query", query, "err", err)
		p.setProviderMetric(span, trustmetrics.Chain, trigger, trustmetrics.ErrDB, err)
		return nil, serrors.Wrap("fetching chains from database", err)
	}

	if o.allowInactive && len(chains) > 0 {
		p.setProviderMetric(span, trustmetrics.Chain, trigger, trustmetrics.Success, nil)
		return chains, nil
	}
	...
	if err != nil {
		...
		p.setProviderMetric(span, trustmetrics.Chain, trigger, result, err)
	...
	if err := p.Recurser.AllowRecursion(o.client); err != nil {
		p.setProviderMetric(span, trustmetrics.Chain, trigger, trustmetrics.ErrNotAllowed, err)
	...
	if o.server == nil {
		if o.server, err = p.Router.ChooseServer(ctx, query.IA.ISD()); err != nil {
			p.setProviderMetric(span, trustmetrics.Chain, trigger, trustmetrics.ErrInternal, err)
	...
	if len(chains) == 0 {
		p.setProviderMetric(span, trustmetrics.Chain, trigger, trustmetrics.ErrNotFound, nil)
		return nil, nil
	}
	p.setProviderMetric(span, trustmetrics.Chain, trigger, trustmetrics.Success, nil)
	return chains, nil
...
func (p FetchingProvider) NotifyTRC(ctx context.Context, id cppki.TRCID, opts ...Option) error {
	...
	if err != nil {
		p.setProviderMetric(span, trustmetrics.NotifyTRC, trigger, trustmetrics.ErrDB, err)
		return err
	}
	if trc.IsZero() {
		err := serrors.New("no TRC for ISD present", "isd", id.ISD)
		p.setProviderMetric(span, trustmetrics.NotifyTRC, trigger, trustmetrics.ErrNotFound, err)
		return err
	}
	if trc.TRC.ID.Base != id.Base {
		p.setProviderMetric(span, trustmetrics.NotifyTRC, trigger, trustmetrics.ErrValidate, nil)
		return serrors.New("base number mismatch", "expected", trc.TRC.ID.Base, "actual", id.Base)
	}
	if id.Serial <= trc.TRC.ID.Serial {
		p.setProviderMetric(span, trustmetrics.NotifyTRC, trigger, trustmetrics.Success, nil)
		return nil
	}
	if err := p.Recurser.AllowRecursion(o.client); err != nil {
		p.setProviderMetric(span, trustmetrics.NotifyTRC, trigger, trustmetrics.ErrNotAllowed, err)
		return serrors.Wrap("recursion not allowed", err)
	}
	if o.server == nil {
		if o.server, err = p.Router.ChooseServer(ctx, id.ISD); err != nil {
			p.setProviderMetric(span, trustmetrics.NotifyTRC, trigger, trustmetrics.ErrInternal, err)
	...
	for serial := trc.TRC.ID.Serial + 1; serial <= id.Serial; serial++ {
		...
		if err := fetched.Verify(&trc.TRC); err != nil {
			p.setProviderMetric(span, trustmetrics.NotifyTRC, trigger, trustmetrics.ErrVerify, err)
...
func (p FetchingProvider) setProviderMetric(span opentracing.Span,
	reqType, trigger, result string, err error) {
	if requests := p.Requests; requests != nil {
		metrics.CounterInc(requests(reqType, trigger, result))
	}
