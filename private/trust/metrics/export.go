...
type Metrics struct {
	ProviderRequests   func(reqType, trigger, result string) metrics.Counter
	RPCFetches         func(reqType, trigger, peer, result string) metrics.Counter
...
func New(opts ...metrics.Option) Metrics {
	...
	return Metrics{
		ProviderRequests: func(reqType, trigger, result string) metrics.Counter {
			return providerRequests.WithLabelValues(reqType, trigger, result)
		},
		RPCFetches: func(reqType, trigger, peer, result string) metrics.Counter {
			return rpcFetches.WithLabelValues(reqType, trigger, peer, result)
		},
		SignerSignatures: func(result string) metrics.Counter {
...
// TODO: this never seems used, was that the case before?
// CtxWithTrigger returns a new context, based on ctx, that embeds argument trigger.
func CtxWithTrigger(ctx context.Context, trigger string) context.Context {
	return context.WithValue(ctx, triggerKey, trigger)
}

// TriggerFromCtx returns the trigger embedded in ctx if one exists, or "application"
// otherwise.
